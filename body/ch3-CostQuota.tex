...
Given an allocation $\{x_t\}_{t=1}^T$, the social welfare is defined as the total value derived from the buyers minus the packing cost incurred by the seller:
\[
...
\begin{assumption}[Bounded Valuations]\label{assumption-bounded-value}
    For each buyer $t \in [T]$ from group $j_t \in [K]$, the per-unit value $v_t$ lies within a known bounded interval $[1, \theta_{j_t}]$, where $\theta_{j_t} \geq 1 $ is a constant. Without loss of generality, we assume $ \theta_0 = 1 $ and $ \theta_0 \leq \theta_1 \leq \theta_2 \leq \dots \leq \theta_K $.
\end{assumption}
...
\begin{itemize}
    \item A \emph{setup} $S = \bigl(B,\,M,\,f,\,\{\theta_j\}_{j=1}^K\bigr)$ that includes information known a priori to the seller, where:
          \begin{itemize}
              ...
              \item $f : [0,B] \to \mathbb{R}_{+}$ is the packing cost function,
...
\]
Based on the marginal cost, we define the \emph{packing function}
\[
    B(v) =
    \begin{cases}
    ...
    \end{cases}
\]
which represents the maximum number of units that can be profitably packed at a marginal cost not exceeding $v$.

From $ B(v) $, we can further define the \emph{minimum packing level} $ \underline{B} $ and the \emph{maximum packing level} $ \overline{B} $ as follows.
\[
    \underline{B} =
    \begin{cases}
    ...
    \end{cases}
\]
denotes the maximum number of resources packed when all arrivals have value $1$. Conversely, the \emph{maximum packing level}
\[
    \overline{B} =
    \begin{cases}
        f'^{-1}(\theta_K), & \theta_K\in (\underline{c}, \overline{c}), \\
        B,                 & \overline{c} \leq \theta_K,
    \end{cases}
\]
denotes the maximum number of resources packed when all arrivals have value $\theta_K$.
Next, we extend the packing cost function to the entire non-negative real line by defining
\[
    \bar{f}(y) =
    \begin{cases}
        f(y),   & y\in [0, B],       \\
...
\(C_j(v) = B(v) - \sum_{i=1}^{j-1} m_i\) is the remaining packing capacity after satisfying the quota requirements of groups \(1\) through \(j-1\), and
...
We begin by presenting a posted price mechanism that first satisfies the quota requirements for each group and then allocates the remaining resources based on a dynamically adjusted posted price threshold. The detailed procedure is outlined in Algorithm \ref{alg_cost_quota_posted_price}.

\begin{algorithm}
    \caption{Posted Price Mechanism with Quota Constraints}
    \label{alg_cost_quota_posted_price}
    \begin{algorithmic}[1]
        \Require Budget $B$, quotas $\{m_j\}_{j\in[K]}$, pricing function $\phi(\cdot)$
        \State Initialize $u^0 \gets 0$ and $u_j^0 \gets 0$ for all $j\in[K]$
        \While{agent $t$ arrives}
        \State $y_t \gets 0$, $x_t \gets 0$
        \If{$u_{j_t}^{t-1} < m_{j_t}$} \Comment{Satisfy quota for class $j_t$ first}
        \State $y_t \gets \min\{r_t,\, m_{j_t}-u_{j_t}^{t-1}\}$
        \State Update $u_{j_t}^{t} \gets u_{j_t}^{t-1}+y_t$
        \EndIf
        \If{$v_t \ge \phi(u^{t-1})$} \Comment{Allocate remaining resource by posted-price threshold}
        \State $x_t \gets \min\left\{\arg\max_{a\in[0,\,r_t-y_t]}\left( a\,v_t - \int_{u^{t-1}}^{u^{t-1}+a}\phi(\eta)\,d\eta \right),\, B-M-u^{t-1}\right\}$
        \EndIf
        \State Update the cumulative non-quota allocation: $u^{t} \gets u^{t-1}+x_t$
        \State Update the allocation of agent $t$: $x_t \gets x_t + y_t$
        \EndWhile
    \end{algorithmic}
\end{algorithm}
...
\begin{theorem}\label{therem-sufficient-cost-quota}
    ...
    \begin{equation}\label{eq:phi-incremental-condition}
        \begin{cases}
            ...
            \varphi(\omega) = 1, \varphi(\overline{B}) \geq \theta_K
        \end{cases}
    \end{equation}
\end{theorem}
...
\begin{definition}
    \textbf{Hard Instance Family for Quota with Packing Costs.}
...
\begin{theorem}\label{theorem-quota-general-necessary-conditions}
    \textbf{Necessary Conditions for Quota with Packing Costs.}
...
\begin{proof}
    ...
    Fix $v \in [\theta_{j-1},\theta_j]$ for some $j\in[K]$ and consider the truncated hard instance $\mathcal{I}^{\mathrm{Q}}_v$. By construction, the maximum value in $\mathcal{I}^{\mathrm{Q}}_v$ is $v$, and the maximum profitable total packing level is therefore $B(v)$.
