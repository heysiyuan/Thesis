...
\begin{theorem}\label{theorem-gfq-general-necessary-conditions}
    \textbf{Necessary Conditions for Quota with Production Costs.}
    If there exists an $ \alpha $-competitive online algorithm that also satisfies the GFQ requirement, then there must exist a parameter $ \omega $ that satisfies
    \begin{equation}\label{equation-gfq-omega}
        \begin{aligned}
    ...
    \begin{equation}\label{equation-gfq-utilization}
        \begin{aligned}
            \begin{cases}
...
\begin{proof}
    ...
    Thus, to guarantee $ \alpha $-competitiveness against the arrival instance $ I^{\GFQ}_v $, the following differential equation has to hold for all $ v \in [\theta_{j-1}, \theta_j] $:
    \begin{align*}
    ...
    Putting it together, if there exists an $ \alpha $-competitive online algorithm that satisfies the GFQ requirement, then there must exist a non-decreasing utilization function $ \psi(v) $ that satisfies the above equation with boundary conditions $ \psi(1) = \omega $ and $ \psi(\theta_K) \leq \bar{B} $, as stated in equation \eqref{equation-gfq-utilization}.
\end{proof}
