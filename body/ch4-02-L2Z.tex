...
\begin{proof}
    ...
    \paragraph{Case 1.} If $ v_t \geq \eta^{t-1}_{j_t} $ but $ v_t < \lambda^{t-1} $, then the dual change is
    \begin{align*}
    ...
    {\color{red}
    In this case, there is only one active allocation source, namely the class-specific allocation, so $x_t = x_t^{j_t}$. From the specified update rule for $\eta_{j_t}^t$ we have
    \[
        \Delta D_t
        = m_{j_t}(\eta^t_{j_t} - \eta^{t-1}_{j_t})
        = m_{j_t}\bigl(\eta^{t-1}_{j_t}+\beta^t_{j_t}\bigr)
        \left(\exp\left(\frac{\alpha}{m_{j_t}}x_t^{j_t}\right)-1\right).
    \]
    }

    % If $x_t^{j_t} = 0$, then $x_t = 0$, which implies $\Delta P_t = 0$ and, from the expression above, $\Delta D_t = 0$. Hence $\Delta P_t \ge \Delta D_t / \alpha$ holds trivially. Therefore we only need to consider the nontrivial case $x_t^{j_t} > 0$.

    % By construction of the algorithm, $x_t^{j_t}$ maximizes the corresponding one-dimensional objective. For an interior maximizer $x_t^{j_t} > 0$, the first-order optimality condition yields
    % \[
    %     \frac{v_t + \beta^t_{j_t}}{\eta^{t-1}_{j_t} + \beta^t_{j_t}}
    %     = \exp\!\left(\frac{\alpha}{m_{j_t}} x_t^{j_t}\right),
    % \]
    % or equivalently
    % \[
    %     v_t + \beta^t_{j_t}
    %     = \bigl(\eta^{t-1}_{j_t} + \beta^t_{j_t}\bigr)
    %     \exp\!\left(\frac{\alpha}{m_{j_t}} x_t^{j_t}\right).
    % \]

    % Since $x_t = x_t^{j_t}$ in Case~1, the primal increment can be written as
    % \begin{align*}
    %     \Delta P_t
    %      & = (v_t + \beta^t_{j_t}) x_t
    %     = (v_t + \beta^t_{j_t}) x_t^{j_t}                 \\
    %      & = \bigl(\eta^{t-1}_{j_t} + \beta^t_{j_t}\bigr)
    %     \exp\!\left(\frac{\alpha}{m_{j_t}} x_t^{j_t}\right)
    %     x_t^{j_t}.
    % \end{align*}
    % Therefore, the desired inequality $\Delta P_t \ge \Delta D_t / \alpha$ is equivalent to
    % \[
    %     \bigl(\eta^{t-1}_{j_t} + \beta^t_{j_t}\bigr)
    %     \exp\!\left(\frac{\alpha}{m_{j_t}} x_t^{j_t}\right)
    %     x_t^{j_t}
    ...
    %     \left(\exp\!\left(\frac{\alpha}{m_{j_t}}x_t^{j_t}\right)-1\right).
    % \]
    % Since $\eta^{t-1}_{j_t} + \beta^t_{j_t} > 0$, we can divide both sides by this positive factor. Let
    % \[
    %     y := \frac{\alpha}{m_{j_t}} x_t^{j_t} \ge 0.
    % \]
    % The inequality reduces to
    % \[
    %     e^{y} \cdot \frac{m_{j_t}}{\alpha} y
    %     \;\ge\;
    %     \frac{m_{j_t}}{\alpha}\bigl(e^{y} - 1\bigr),
    % \]
    % or, after cancelling the common scalar $m_{j_t}/\alpha > 0$,
    % \[
    %     e^{y} y \;\ge\; e^{y} - 1.
    % \]

    % Define $g(y) := e^{y} y - e^{y} + 1 = e^{y}(y-1) + 1$. Then
    % \[
    %     g(0) = 0, \qquad
    %     g'(y) = e^{y}(y-1) + e^{y} = e^{y} y \ge 0 \quad \forall\, y \ge 0.
    % \]
    % Hence $g$ is nondecreasing on $[0,\infty)$ and $g(y) \ge g(0) = 0$ for all $y \ge 0$, which implies
    % \[
    %     e^{y} y \;\ge\; e^{y} - 1 \quad \forall\, y \ge 0.
    % \]
    % Therefore $\Delta P_t \ge \Delta D_t / \alpha$ holds in Case~1.
    % }
