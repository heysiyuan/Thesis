...
\begin{proof}
    ...
    We prove that the primal solution produced by Algorithm \ref{alg_LinearToZero} is feasible, i.e., $ \sum_{t=1}^T x_t \le B $, for any arrival sequence. We use induction on time $ t $ and the idea of \emph{future feasibility}. An allocation prefix is future-feasible if, even under the worst-case future sequence of arrivals, the total allocation remains within the budget $ B $.
    ...
    \[
        \Gamma_i^t(\eta_j^t, \lambda^t) = Z^t + \sum_{j\in[K]} \Delta(\eta_j^t) + \bar{\Delta}(\lambda^t),
    \]
    ...
    \textbf{Base Case $ (t = 0) $.} At $ t = 0 $, we have $ Z^0 = Z_j^0 = 0 $ for all $ j\in [K]$, and the algorithm initializes $ \eta_j^0 = 1 $ for all $ j $ and $ \lambda^0 = 1 $. Fix any $ i\in[K] $ and recall that the thresholds $\{v^*_{k,i}\}_{k\in[K]}$ are given by Theorem \ref{theorem-linear2zero-soft-gfq-upper-bound}. By the definition of $\Gamma_i^t$ (and using $Z^0=0$),
    \begin{align*}
        \Gamma_i^0
         & = \sum_{j\in[K]} \Delta(\eta_j^0) + \bar{\Delta}(\lambda^0)                                                                                                 \\
         & = \sum_{j\ne i}\Bigl(\frac{m_j}{\alpha}\ln\Bigl(\tfrac{v^*_{j,i}+\beta_j}{1+\beta_j}\Bigr)+\frac{m_j}{\alpha}\ln\Bigl(\tfrac{\theta}{v^*_{j,i}}\Bigr)\Bigr)
        + \Bigl(\frac{B-M+m_i}{\alpha}\ln\Bigl(\tfrac{v^*_{i,i}+\beta_i}{1+\beta_i}\Bigr)+\frac{B-M+m_i}{\alpha}\ln\Bigl(\tfrac{\theta}{v^*_{i,i}}\Bigr)\Bigr).
    \end{align*}

    From the design of Algorithm \ref{alg_LinearToZero}, there is a reservation $ A_j $ for each class $ j\in [K] $. Therefore, to show that the prefix at time $ t = 0 $ is future-feasible, it suffices to show that $ \Gamma_i^0 + \sum_{j\in[K]} A_j \leq B $ for all $ i \in [K] $.
    ...
    Fixing $ i \in [K] $ and taking $ \ell = i $, the first two constraints imply
    \[
        \sum_{j\in[K]} A_j
        + \frac{B-M+m_i}{\alpha}\ln\Bigl(\tfrac{v^*_{i,i}+\beta_i}{1+\beta_i}\Bigr)
        + \sum_{j\ne i} \frac{m_j}{\alpha}\ln\Bigl(\tfrac{v^*_{j,i}+\beta_j}{1+\beta_j}\Bigr)
        = \sum_{j\in[K]} m_j = M.
    \]
    Substituting this identity into the expression for $\Gamma_i^0+\sum_{j\in[K]}A_j$ and then applying the last inequality in Theorem \ref{theorem-linear2zero-soft-gfq-upper-bound} yields
    \[
...
\begin{proof}
    ...
    \paragraph{Primal Feasibility.} We will prove primal feasibility by the same idea of future feasibility as in Theorem \ref{theorem-linear2zero-soft-gfq-upper-bound} to show the total allocation never exceeds $ B $. The argument is similar to that in Theorem \ref{theorem-linear2zero-soft-gfq-upper-bound}, and thus omitted here for brevity. The corresponding max-potential allocation expressions are given by
    \[
        \Gamma_i^t(\bar\eta_j^t, \bar\lambda^t) = Z^t + \sum_{j\in[K]} \Delta(\bar\eta_j^t) + \bar{\Delta}(\bar\lambda^t),
    \]
    where $ \Delta(\bar\eta_j^t) $ and $ \bar{\Delta}(\bar\lambda^t) $ are defined as follows:
    \[
        \Delta(\bar\eta_j^t) = \begin{cases}
            \frac{m_j}{\alpha} \ln\Bigl(\frac{v^*_{j,i}}{\bar\eta_j^t}\Bigr) + \frac{m_j}{\alpha} \ln\Bigl(\frac{\theta-\bar\beta_j}{v^*_{j,i}-\bar\beta_j}\Bigr) & \text{if } \bar\eta_j^t < v^*_{j,i},    \\
            \frac{m_j}{\alpha} \ln\Bigl(\frac{\theta-\bar\beta_j}{\bar\eta_j^t-\bar\beta_j}\Bigr),                                                                & \text{if } \bar\eta_j^t \geq v^*_{j,i},
        \end{cases}
    \]
    \[
        \bar{\Delta}(\bar\lambda^t) = \begin{cases}
            \frac{B-M}{\alpha} \ln\Bigl(\frac{v^*_{i,i}}{\bar\lambda^t}\Bigr) + \frac{B-M}{\alpha} \ln\Bigl(\frac{\theta-\bar\beta_i}{v^*_{i,i}-\bar\beta_i}\Bigr) & \text{if } \bar\lambda^t < v^*_{i,i},    \\
            \frac{B-M}{\alpha} \ln\Bigl(\frac{\theta-\bar\beta_i}{\bar\lambda^t-\bar\beta_i}\Bigr),                                                                & \text{if } \bar\lambda^t \geq v^*_{i,i}.
        \end{cases}
    \]
\end{proof}
...
\begin{proof}
    ...
    \paragraph{Primal Feasibility.} We again employ the future feasibility method to show that the total allocation does not exceed $B$. The argument is similar to that in the full proof of Theorem \ref{theorem-linear2zero-soft-gfq-upper-bound}. The corresponding max-potential allocation expressions in this case are defined below.
    For each reference class $i\in[K]$, define
    \[
        \Gamma_i^t(\tilde\eta_j^t, \lambda^t) = Z^t + \sum_{j\in[K]} \Delta(\tilde\eta_j^t) + \bar{\Delta}(\lambda^t),
    \]
    where $ \Delta(\tilde\eta_j^t) $ and $ \bar{\Delta}(\lambda^t) $ are defined as follows:
    \[
        \Delta(\tilde\eta_j^t) = \begin{cases}
            \frac{m_j}{\alpha}\ln\Bigl(\frac{v^*_{j,i}+\beta_j}{\tilde\eta_j^t+\beta_j}\Bigr) + \frac{m_j}{\alpha}\ln\Bigl(\frac{\theta-\bar\beta_j}{v^*_{j,i}-\bar\beta_j}\Bigr) & \text{if } \tilde\eta_j^t < v^*_{j,i},   \\
            \frac{m_j}{\alpha}\ln\Bigl(\frac{\theta-\bar\beta_j}{\tilde\eta_j^t-\bar\beta_j}\Bigr),                                                                               & \text{if } \tilde\eta_j^t \ge v^*_{j,i},
        \end{cases}
    \]
    \[
        \bar{\Delta}(\lambda^t) = \begin{cases}
            \frac{B-M}{\alpha}\ln\Bigl(\frac{v^*_{i,i}+\beta_{i}}{\lambda^t+\bar\beta_{i}}\Bigr) + \frac{B-M}{\alpha}\ln\Bigl(\frac{\theta-\bar\beta_{i}}{v^*_{i,i}-\bar\beta_{i}}\Bigr) & \text{if } \lambda^t < v^*_{i,i},   \\
            \frac{B-M}{\alpha}\ln\Bigl(\frac{\theta-\bar\beta_{i}}{\lambda^t}\Bigr),                                                                                                     & \text{if } \lambda^t \ge v^*_{i,i}.
        \end{cases}
    \]
...
The proofs of Theorem \ref{theorem-zero2linear-soft-quota-necessary} and Theorem \ref{theorem-linear2linear-soft-quota-necessary} follow the same structure and reasoning as above, adjusted for the specific forms of the penalty functions and optimal allocations.